...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  for (int i = 0; i < h; i++) {
    newBoard->board[i] = realloc(newBoard->board[i], w * sizeof(newBoard->board[i]));
  }
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
...
//check whether board point is valid
int checkValid(board_t * b, int x, int y) {
...
int countMines(board_t * b, int x, int y) {
  //WRITE ME!
  int count = 0;
  if (checkValid(b, x - 1, y - 1) == 1 && IS_MINE(b->board[y - 1][x - 1])) {
    count++;
  }
  if (checkValid(b, x, y - 1) == 1 && IS_MINE(b->board[y - 1][x])) {
    count++;
  }
  if (checkValid(b, x + 1, y - 1) == 1 && IS_MINE(b->board[y - 1][x + 1])) {
    count++;
  }
  if (checkValid(b, x - 1, y) == 1 && IS_MINE(b->board[y][x - 1])) {
    count++;
  }
  if (checkValid(b, x + 1, y) == 1 && IS_MINE(b->board[y][x + 1])) {
    count++;
